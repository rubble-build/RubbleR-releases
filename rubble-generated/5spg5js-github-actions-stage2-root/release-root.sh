#!/var/lib/rubble/store/li/fl/out-lifled7ii45rfxevjn4zzhsjiduayqc3qoinypo3xoznu4pereja-bootstrap-env/bin/bash
set -euo pipefail
IFS=$'\n\t'
...
script_dir="$(builtin cd "$('/var/lib/rubble/store/li/fl/out-lifled7ii45rfxevjn4zzhsjiduayqc3qoinypo3xoznu4pereja-bootstrap-env/bin/dirname' "${BASH_SOURCE[0]}")" && builtin pwd)"
...
[[ -x '/usr/bin/gh' ]] || fail "declared GitHub CLI is unavailable: '/usr/bin/gh'"
...
'/var/lib/rubble/store/li/fl/out-lifled7ii45rfxevjn4zzhsjiduayqc3qoinypo3xoznu4pereja-bootstrap-env/bin/mkdir' -p "${release_dir}"
...
release_name="$( '/var/lib/rubble/store/li/fl/out-lifled7ii45rfxevjn4zzhsjiduayqc3qoinypo3xoznu4pereja-bootstrap-env/bin/date' -u +'%Y%m%d%H%M%S')"
release_tag="${release_name}-${run_id}-${run_attempt}"
log "creating GitHub release ${release_name} for ${root_id}"
'/usr/bin/gh' release create "${release_tag}" \
  --repo "${repository}" \
  --title "${release_name}" \
