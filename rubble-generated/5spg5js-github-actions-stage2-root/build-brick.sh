...
[[ -n "${RUBBLE_SERVER_CONFIG:-}" ]] || fail "RUBBLE_SERVER_CONFIG is required"
[[ -n "${RUBBLE_SERVER_CREDENTIALS:-}" ]] || fail "RUBBLE_SERVER_CREDENTIALS is required"
...
log "writing complete authentication files from repository secrets"
umask 077
printf '%s' "${RUBBLE_SERVER_CONFIG}" >"${config_file}"
printf '%s' "${RUBBLE_SERVER_CREDENTIALS}" >"${credentials_file}"
unset RUBBLE_SERVER_CONFIG RUBBLE_SERVER_CREDENTIALS
