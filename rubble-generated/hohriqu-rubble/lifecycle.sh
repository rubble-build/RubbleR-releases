...
export RUBBLE_ROOT_ID='hohriqulrt66q27rx6zra3ftyp6sev6jhnmmpmyhefrss5iewv2a-rubble'
...
if [[ "${RUBBLE_ON_RUNNER:-}" == 1 ]]; then
  case "${RUBBLE_BUILD_PLATFORM}" in
    ...
    'x86_64-linux')
      ...
      rubble_runner_commands['bash']='/usr/bin/bash'

      rubble_runner_commands['date']='/usr/bin/date'

      rubble_runner_commands['dirname']='/usr/bin/dirname'

      rubble_runner_commands['gh']='/usr/bin/gh'

      rubble_runner_commands['mkdir']='/usr/bin/mkdir'

      rubble_runner_commands['node']='/usr/local/bin/node'
