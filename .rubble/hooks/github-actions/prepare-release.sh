...
echo "$(rubble-exec date '+%Y%m%d-%H%M%S')-${brick_inventory[::root]}" > title

if [[ "${brick_inventory[::root]}" = *-rubble-inventory-bundle-import ]]; then
  rubble-exec cp -v "${brick_inventory[${brick_inventory[::root]}:out]}" "${RUBBLE_RELEASE_DIR}/assets/${brick_inventory[::root]}.sh"
else
...
for file in *; do
  [[ "$file" = @(*.SHA256|title|tag|notes.md) ]] && continue
